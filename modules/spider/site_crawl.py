...
structured=True（默认）：入口页先跑 sidebar_parser 解析导航目录；解析出
章节结构则按「一级章节一个子目录、每页一个 md」输出，并在 output_dir 写
_index.json（完整侧边栏结构 + 页面清单 + 失败清单）；解析不到侧边栏时
回落同域 BFS，并在结果 data 中注明 fallback_bfs=true。
...
def crawl_site(url: str, output_dir: Path, max_pages: int = 200, interval: float = 1.0,
    ...
    if structured:
        ...
        result = _crawl_structured(url, entry_html, output_dir, max_pages, interval, fetch)
        if result is not None:
            return result
        info("未解析到侧边栏目录，回落同域 BFS 爬取")
        return _crawl_bfs(url, output_dir, max_pages, interval, fetch, fallback=True)

    return _crawl_bfs(url, output_dir, max_pages, interval, fetch)


def _crawl_structured(url: str, entry_html: str, output_dir: Path,
                      max_pages: int, interval: float, fetch) -> dict | None:
    ...
    total = min(len(queue), max_pages)
    for index, (target, chapter) in enumerate(queue[:max_pages], start=1):
        try:
            # 入口页复用已抓取的 HTML，省一次请求
            page_html = entry_html if target == entry else fetch.fetch_html(target)
            text, title = fetch.html_to_md(page_html)
...
def _crawl_bfs(url: str, output_dir: Path, max_pages: int, interval: float,
               fetch, fallback: bool = False) -> dict:
